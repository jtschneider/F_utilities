...
function Diag_real_skew(M)
    ...
    for iiter = 1:(convert(Int64, N_f))
        if (Schur_blocks_i[2*iiter, 2*iiter-1] >= 0.0)
...
function Energy_fermion(Mat, D, U)
    N_f = convert(Int64, size(Mat, 1) / 2.0)
...
function VN_entropy(M)
    ...
    D, U = eig((M + M') / 2.0)   #Se voglio fare con eig
    D = diagm(real(D))
...
function Bogoliubov_matrix(Jx, Jy, lambda, N_f)
    ...
    for kiter = 1:N_f
        ...
        theta[kiter] =
            0.5 * atan(
                -(
                    ((Jx - Jy) * sin(2 * pi * k / N_f)) /
                    ((Jx + Jy) * cos(2 * pi * k / N_f) + lambda)
                ),
            )
    end
...
function Build_thermal_gamma(Diag_H, U_H, conserved_energy)
    ...
    println(
        "Thermal state with energy precision of:",
        abs(temp_energy - conserved_energy),
        " and beta: ",
        beta,
    )
...
function E_contribute(U, i, k)
    N_f = convert(Int64, size(U, 1) / 2.0)
    return real(
        0.5 * (
            U[i, k] * conj(U[i, k]) +
            U[i, k+N_f] * conj(U[i, k+N_f]) +
            U[i+N_f, k] * conj(U[i+N_f, k]) +
            U[i+N_f, k+N_f] * conj(U[i+N_f, k+N_f])
        ),
    )
end
#####
...
function exchange(U, column1, column2, element, site)
    element = element - 1
    N_red = convert(Int64, size(U, 1) / 2.0)
...
function diagonalise_block(γi, starting_site, dimension)
    ...
    γ_finale = (γ_finale + γ_finale') / 2.0
...
function trunker(Γ, m)
    ...
    for i = (m+1):(N_f-(m+1))
        ...
        ind_a = max(1, i - convert(Int64, N_f / 2.0) + 1)
        ...
        ind_b = i + max(1, N_f - i + 1 - convert(Int64, N_f / 2.0))
        Γ[ind_b, :] = 0
        Γ[:, ind_b] = 0
        Γ[ind_b+N_f, :] = 0
        Γ[:, ind_b+N_f] = 0
...
function reduce_bond_dimension_2(Γ, L1, L2, m)
    ...
    for k = 1:N_momenti
        ...
        Γ_new[k, :, :], v[1, k] =
            turbo_trunker(Γ_new[k, :, :], convert(Int64, L2 - v[2, k] - 1))
    end

    Tot_conserved_mode = (L1 * L2) / 2.0
...
function Purification(Γ_mixed)
    N = convert(Int64, size(Γ_mixed, 1) / 2.0)
...
function Unitary_composite(λ)
    ...
    size_y = convert(Int64, (d * (d - 1)) / 2.0)
    ...
    for miter = 1:d
        for niter = (miter+1):d
            ...
            Y_matrix =
                -im * (onb[miter, :] * onb[niter, :]') +
                im * (onb[niter, :] * onb[miter, :]')
    ...
    while (check)
        ...
        if (mod(iiter, 10) == 0)
            println(
                ...
                Δ,
                "Tot Coh: ",
                Sum_2_site_coherences(Γ),
            )
        end
    end

    return U_tot, (Γ + Γ') / 2.0
end

function Minimise_coherence_4_TI(Γ)
    n = convert(Int64, size(Γ, 1) / 2.0) #Questo deve essere un multiplo di 4
    ...
    for iiter = 1:(convert(Int64, n / 4.0))
        UA =
            Ω' *
            Inject_gamma(
                convert(Array{Complex{Float64},2}, eye(2 * n)),
                U2,
                (iiter - 1) * 4 + 1,
            ) *
            Ω *
    ...
    for iiter = 1:(convert(Int64, n / 4.0))
        UD =
            Ω' *
            Inject_gamma(convert(Array{Complex{Float64},2}, eye(2 * n)), U2, iiter * 4) *
            Ω *
            UD
    end
    Γ = UD * Γ * UD'
...
function RBD(Γ_in, L1, L2, m)
    ...
    for siter = 1:(L2-m)
        for iiter = 1:N_momenti
            Γ[iiter, :, :], U[iiter, :, :] = diagonalise_block(
                Γ[iiter, :, :],
                convert(Int64, v[2, iiter] + 1),
                convert(Int64, 2 * (m + siter) - v[2, iiter]),
            )
...
function RBD_uno_per_volta(Γ_in, L1, L2, mdL)
    ...
    for siter = 1:(L2-mdL)
        for iiter = 1:N_momenti
            Γ[iiter, :, :], U[iiter, :, :] =
                diagonalise_block(Γ[iiter, :, :], 1, 2 * (mdL + siter))
            U_Γ[iiter, :, :] = U_Γ[iiter, :, :] * U[iiter, :, :]
...
function RBD_nuovo(Γ_in, L1, L2, mdL)
    ...
    for L2iter = 1:(L2-mdL)
        for k = 1:N_momenti
            Γ[k, :, :], U[k, :, :] = diagonalise_block(
                Γ[k, :, :],
                convert(Int64, v[2, k]),
                2 * (mdL + L2iter) - v[2, k] + 1,
            )
            U_Γ[k, :, :] = U_Γ[k, :, :] * U[k, :, :]
...
function Build_fourier_matrix(N)
    ...
    for k = 1:N
        for x = 1:N
            U[k, x] = exp(-im * 2 * pi * (k - (N - 1) / 2.0) * (x - (N - 1) / 2.0) / N)
        end
    end
...
function Fermionic_Negativity(Λ, N_A)
    ...
    Tp = [
        eye(2 * N_A) zeros(Int64, 2 * N_A, 2 * N_B)
        zeros(Int64, 2 * N_B, 2 * N_A) im*eye(2 * N_B)
    ]
    Tm = [
        eye(2 * N_A) zeros(Int64, 2 * N_A, 2 * N_B)
        zeros(Int64, 2 * N_B, 2 * N_A) -im*eye(2 * N_B)
    ...
    ZZZ = tilde_det((I - γ * γ) / 2.0)
    TR = tilde_det(sqrt((I + im * γx) / 2.0) + sqrt((I - im * γx) / 2.0))
...
function Reduce_double_gamma(Λ, s1, d1, s2, d2)
    ...
    γ_reduced[((1:(2*d2))+2*d1), ((1:(2*d2))+2*d1)] =
        γ[((1:(2*d2))+2*s2-2), ((1:(2*d2))+2*s2-2)]
...
function Modes_spatial_contribution(Λ)
    ...
    for s = 1:N
        for e = 1:N
            contributions[s, e] +=
                real(U[s, e] * conj(U[s, e]) + U[s, e+N] * conj(U[s, e+N]))
            contributions[s, e] +=
                real(U[s+N, e] * conj(U[s+N, e]) + U[s+N, e+N] * conj(U[s+N, e+N]))
        end
    end
    ...
    for s = 1:N
        for e = 1:N
            contributions[s, e] +=
                real(U[s, e] * conj(U[s, e]) + U[s, e+N] * conj(U[s, e+N]))
            contributions[s, e] +=
                real(U[s+N, e] * conj(U[s, e]) + U[s+N, e+N] * conj(U[s, e+N]))
        end
    end
    contributions = contributions
...
function myhist(data, min, max, nbins)
    ...
    for k = 1:nbins
        ...
        bin[k] = start + delta / 2.0 # Centre of the bin
...
function Ent_cont(Λ)
    ...
    for i = 1:N
        for k = 1:N
            p[i, k] =
                0.5 * (
                    (O[2*i-1, 2*k-1] * conj(O[2*i-1, 2*k-1])) +
                    (O[2*i, 2*k-1] * conj(O[2*i, 2*k-1])) +
                    (O[2*i-1, 2*k] * conj(O[2*i-1, 2*k])) +
                    (O[2*i, 2*k] * conj(O[2*i, 2*k]))
                )
        end
    end
...
function xc(Exact_E, c)
    ...
    for n = 1:(N_points-1)
        renorm[n] =
            ((log(abs(Exact_E[n] + c))) - minimum(log.(abs.(Exact_E[:] + c)))) /
            (maximum(log.(abs.(Exact_E[:] + c))) - minimum(log.(abs.(Exact_E[:] + c))))
    end
    der_seconde = zeros(Float64, N_points - 3)
    for n = 2:(N_points-2)
        der_seconde[n-1] = (renorm[n+1] - 2 * renorm[n] + renorm[n-1]) / 2.0
    end
...
function GDE(g_ferm_i, U_diag_f_Q)
    return (
        U_diag_f_Q *
        Project_diagonals(U_diag_f_Q' * g_ferm_i * U_diag_f_Q, 0) *
        U_diag_f_Q'
    )
end
...
function Diag_real_skew_eig(M)
    ...
    return real.(
        im * block_antidiag * mat_sort * diagm(DD) * (im * block_antidiag * mat_sort)'
    ),
    UOU'
end
