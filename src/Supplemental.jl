...
function Reduce_gamma2D(M, Lx, Ly, lx, ly, r)
    ...
    if !((r[1] + lx) <= Lx && (r[2] + ly) <= Ly)
        error(
            "In Reduce_gamma2D the selected partition exceeds the boundaries of the system",
        )
    end
    ...
    for i = 0:(ly-1)
        for j = 0:(ly-1)
            redgammaTL[(i*lx+1):((i+1)*lx), (j*lx+1):((j+1)*lx)] = MTL[
                T1D(Lx, Ly, (r + [0, i])):(T1D(Lx, Ly, (r + [0, i]))+lx-1),
                T1D(Lx, Ly, (r + [0, j])):(T1D(Lx, Ly, (r + [0, j]))+lx-1),
            ]
            redgammaTR[(i*lx+1):((i+1)*lx), (j*lx+1):((j+1)*lx)] = MTR[
                T1D(Lx, Ly, (r + [0, i])):(T1D(Lx, Ly, (r + [0, i]))+lx-1),
                T1D(Lx, Ly, (r + [0, j])):(T1D(Lx, Ly, (r + [0, j]))+lx-1),
            ]
            redgammaBL[(i*lx+1):((i+1)*lx), (j*lx+1):((j+1)*lx)] = MBL[
                T1D(Lx, Ly, (r + [0, i])):(T1D(Lx, Ly, (r + [0, i]))+lx-1),
                T1D(Lx, Ly, (r + [0, j])):(T1D(Lx, Ly, (r + [0, j]))+lx-1),
            ]
            redgammaBR[(i*lx+1):((i+1)*lx), (j*lx+1):((j+1)*lx)] = MBR[
                T1D(Lx, Ly, (r + [0, i])):(T1D(Lx, Ly, (r + [0, i]))+lx-1),
                T1D(Lx, Ly, (r + [0, j])):(T1D(Lx, Ly, (r + [0, j]))+lx-1),
            ]
        end
